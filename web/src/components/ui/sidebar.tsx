...
function SidebarProvider({
  ...
  // listener inside a component is invisible to that table, so the chord could not be documented in
  // the Settings' Keyboard section, could not be tested, and would keep firing while a dialog owned
  // the keyboard. The binding still exists — it is `toggle-rail`, and the app shell holds this
  // provider open/closed as a controlled component.
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full flex-1 flex-col bg-background lg:peer-data-[variant=inset]:m-2 lg:peer-data-[variant=inset]:ml-0 lg:peer-data-[variant=inset]:rounded-2xl lg:peer-data-[variant=inset]:shadow-sm lg:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
        className
      )}
      {...props}
    />
