 ...
 * carries `status` and `settledAt`, which is the whole of what `SessionHost.reap` compares (ADR
 ...
 *
 * It has to stay in step with the reaper, which is the risk in doing it here — hence the three rules
 * restated rather than paraphrased: only Settled Agent Sessions are ever reaped, a timestamp that
 * will not parse is left alone, and so is one that is missing — because not knowing something's age
...
export function reapableAt(
  ...
  return sessions.filter((session) => {
    ...
    const settledAt = Date.parse(session.settledAt);
