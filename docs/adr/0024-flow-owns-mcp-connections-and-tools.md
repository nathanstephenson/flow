...
OAuth uses one identity per connection and endpoint on the machine. Credentials are stored in a separate file with mode `0600`, never in Settings or Settings responses. Sign-in is available from Settings without an Agent Session. It uses the browser's Flow origin for the callback, five-minute single-use random state, PKCE, and the official SDK's authorization-server discovery and client registration. This also works through a forwarded port or reverse proxy that preserves the browser's Host header. The callback does not require the Flow cookie: OAuth returns from another site, where the Strict cookie is withheld. After state validation and token exchange, a same-origin page navigates back to the initiating Settings page or Agent Session, with a success or failure notice. Existing Backend Sessions still reconnect through Retry. Changed endpoints cannot receive credentials from the old endpoint.

An HTTP connection can also carry custom request headers, for a server that wants a static token or a tenant identifier instead of OAuth. A header value is either a literal string or the name of a Secret. A Secret name keeps the value in the Secret file with mode `0600` and out of Settings responses; a literal value is stored in Settings and is readable by any client holding the token. A Secret-backed header value is always redacted from transcripts and Workflow Executions; a literal one is redacted only when its header name reads as a credential, because redaction is substring replacement and an innocuous value would corrupt unrelated output. A connection whose header names a missing Secret fails rather than connecting without it. Headers are sent before the OAuth token, so on a connection using both, OAuth owns `Authorization`.
