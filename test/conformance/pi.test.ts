 ...
 * the environment). Opt in with: GOODHARNESS_E2E_PI=1 npm test
 *
 * The translation logic itself is covered without credentials in test/backend/pi-mapping.test.ts.
 */
if (process.env["GOODHARNESS_E2E_PI"] !== "1") {
  console.log("# skipping pi conformance (set GOODHARNESS_E2E_PI=1 with pi credentials to run)");
} else {
