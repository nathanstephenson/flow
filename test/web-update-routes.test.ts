...
import { UpdateRefusal } from '../src/daemon/update-api.ts';
...
test('web update routes require existing authentication, same origin, and explicit fixed confirmation', async () => {
  ...
  const updates = {
    ...
    start: async (version: string) => {
      if (version !== status.latestVersion) throw new UpdateRefusal('Release changed');
      starts++;
      return status;
    },
  };
  ...
  try {
    ...
    for (const body of [{}, { confirmed: false }, { confirmed: true }, { confirmed: true, package: 'other' }, { confirmed: true, version: '2.0.0', package: 'other' }]) {
    ...
    const arbitrary = await fetch(url, {
      method: 'POST',
      headers: { cookie: 'flow=secret', 'content-type': 'application/json' },
      body: JSON.stringify({ confirmed: true, version: '9.9.9' }),
    });
    assert.equal(arbitrary.status, 409);
    assert.equal(starts, 0, 'cancelled, extended, or unvalidated-version requests cannot mutate');

    const accepted = await fetch(url, {
      method: 'POST',
      headers: { cookie: 'flow=secret', 'content-type': 'application/json' },
      body: JSON.stringify({ confirmed: true, version: '2.0.0' }),
    });
